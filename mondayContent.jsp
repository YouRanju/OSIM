...
<body onload="initTime(); monday(); imgSrc();" class="mainbody">
	...
	<div id="dd">
		<img id="national" src="img/Korea.png"" alt=""> 현 재 시 간 | <span id="countdown"
			style="display: none"><%= session.getAttribute("today") %></span><br>
		남 은 시 간 | <span id="endTime"></span> <span id="message"> <%= session.getAttribute("mondayList") %>
		</span>
	</div>
	<div id="howMany">
		...
		<span id="youInfo"><%=session.getAttribute("thisMonth")%>/<%=session.getAttribute("thisDate")%>
			...
			%> <%=session.getAttribute("name")%> <%}%> </span> <span id="target">NEXT
			DAY</span>
